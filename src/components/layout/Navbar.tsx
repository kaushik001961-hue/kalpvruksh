...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-4 left-0 right-0 w-[95%] max-w-7xl mx-auto bg-white/95 backdrop-blur-md border border-neutral-200/80 rounded-2xl z-50 transition-all duration-300 shadow-lg hover:shadow-xl">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        {/* 🚀 FIXED: Removed strict h-24 height and added adaptive vertical padding (py-3 sm:py-4) */}
        <div className="flex items-center justify-between gap-4 py-3 sm:py-4">
          
          {/* 1. BRAND LOGO AREA */}
          <Link href="/" className="flex items-center gap-4 shrink-0 group">
            {/* 🚀 FIXED: Ensured the logo image is crisp and bounded cleanly */}
            <img 
              src="/logo.png" 
              alt="Logo" 
      ...
      {isOpen && (
        <div className="absolute top-[105%] left-0 w-full bg-white rounded-2xl border border-neutral-200 shadow-xl px-6 py-8 flex flex-col gap-5 lg:hidden animate-in fade-in slide-in-from-top-4 duration-200 z-40">
          ...
          {/* Mobile Become Volunteer Dropdown Action */}
          <Link
            href="/volunteer/register"
            onClick={() => setIsOpen(false)}
